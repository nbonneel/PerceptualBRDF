...
namespace 
{
  ...
  protected:
    ...
    // Modif JPF - prototype of friend function may not have default arguments with Apple LLVM
  ...
  inline OptionHolder help(char s='h',
			     const char* l="help",
      ...
      isOk_(true)
  {
    ...
    //char* pargs[args.size()]; // not compliant with VStudio
  ...
  operator>>(argstream& s,const ValueHolder<T>& v)
  {
	...
	*(v.value_) = p(*(iter->second));
	...
	{
	  if (jter->second == iter->second) //Nico: bug here. iter is unvalidated since it just got erased - you can't access iter->second here. Crashes Visual Studio in Debug Mode when dealing with string types. Does not crash in Release.
	  {
	    jter->second = s.values_.end();
	  }
	}
	s.options_.erase(iter);  // Nico: I guess should be: iter = s.options_.erase(iter);
      }
      else
      {
	s.isOk_ = false;
