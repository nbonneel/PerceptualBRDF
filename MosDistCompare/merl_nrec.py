...
import struct
import math
import brdf_calculus as brdf_calculus
...
class Merl:
    ...
    def __init__(self, merl_file):
        ...
        with open(merl_file, 'rb') as f:
            ...
            if n[2] == 180:
                length = self.sampling_theta_h * self.sampling_theta_d * 180
                print('Reciprocal BRDF detected...')
            ...
            if n[2] == 180:
                # Duplicate data
                self.brdf = []
                for i in range(3 * self.sampling_theta_h * self.sampling_theta_d):
                    val = struct.unpack_from('180d', data, offset=struct.calcsize('3i') + i * struct.calcsize('180d'))
                    self.brdf += val
                    self.brdf += val
            else:
    ...
    def eval_interp_theta_phi(self, theta_in, phi_in,theta_out,phi_out):
       theta_h, phi_h, theta_d, phi_d = brdf_calculus.std_coords_to_half_diff(theta_in,phi_in,theta_out,phi_out)
    ...
    def __theta_h_idx(self, theta_h):
        ...
        return int(max(0, min(self.sampling_theta_h - 1,
                          math.floor(th))))
    ...
    def __theta_d_idx(self, theta_d):
        ...
        """
        return int(max(0, min(self.sampling_theta_d -1,
    ...
    def __phi_d_from_idx(self, phi_d_idx):
        ...
        """
        return phi_d_idx / self.sampling_phi_d * (2 * math.pi)
    ...
    def __phi_d_idx(self, phi_d):
        ...
        return int(max(0, min(self.sampling_phi_d - 1,
                          math.floor(self.sampling_phi_d * phi_d / (2 * math.pi)))))
