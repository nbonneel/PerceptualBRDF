...
# Main code  : parsing data file and computing all distances for all brdf couples  
...
for i,ligne in enumerate(fd2):
    ...
    if approx != '"METHOD"': # Skipping the first line
        ...
        brdfRefPath = MerlRefDirectory + brdfName + '.titopo'        
        brdfApproxPath = MerlApproxDirectory + brdfName + '.' + liste_types[approx] + '.titopo'      
        brdfApproxName = brdfName + '.' + approx[1:len(approx)-1]

        # Loading files
        brdf1 = titopo_load_full.LoadTitopo(brdfRefPath)
        brdf2 = titopo_load_full.LoadTitopo(brdfApproxPath)
